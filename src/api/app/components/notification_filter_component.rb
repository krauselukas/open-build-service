class NotificationFilterComponent < ApplicationComponent
  def initialize(selected_filter:, content_moderation_beta: false, projects_for_filter: ProjectsForFilterFinder.new.call, groups_for_filter: GroupsForFilterFinder.new.call)
    super

    @content_moderation_beta = content_moderation_beta
  ...
  def notifications_count
    ...
    counted_notifications['reports'] = finder.for_reports.count if @content_moderation_beta
    counted_notifications['workflow_runs'] = finder.for_workflow_runs.count
    counted_notifications['appealed_decisions'] = finder.for_appealed_decisions if @content_moderation_beta
